 ...
 */
class NicoMovieInfoSpec extends Specification {
  "NicoMovie#getTitle" should {
    "case: sm**** URL" in {
      val inputText = "http://www.nicovideo.jp/watch/sm10780722"
      new NicoMovieInfo(inputText).getTitle must equalTo("おちゃめ機能　歌った")
    }
    "case: sm**** URL + α" in {
    ...
    "case: nm**** URL" in {
      ...
      new NicoMovieInfo(inputText).getTitle must equalTo("【初音ミク】アストロノーツ【オリジナル】")
    }
    "case: nm**** URL + α" in {
    ...
    "case: officialMovie(no prefix) URL" in {
      ...
      new NicoMovieInfo(inputText).getTitle must equalTo("将棋電王戦FINAL 第5局 阿久津主税 八段 vs AWAKE")
    }
    ...
    "case: not Nico2 URL" in {
      ...
      new NicoMovieInfo(inputText).getTitle must beEmpty
    }
  }
